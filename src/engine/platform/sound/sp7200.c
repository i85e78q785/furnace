...
void next_writereg(struct SP7200* chip, unsigned char addr, unsigned char val) {
	addr &= 0xf; 
	int ch = addr / 4;
	int idx = addr & 3;
	switch (idx) {
...
void next_render(struct SP7200* chip) {
	int sum = 0;
	for (int i = 0; i < 4; i++) {
		...
		if (chip->chan[i].counter <= 0) {
			...
			if (chip->chan[i].noise && chip->chan[i].noise != 1, 2, 3, 4) {
...
void next_reset(struct SP7200* chip) {
